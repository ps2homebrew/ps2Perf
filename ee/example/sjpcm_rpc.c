...
{
  if (!sjpcm_inited) return;
  sif_call_rpc(&cd0,SJPCM_GETAVAIL,0,(void*)(&sbuff[0]),64,(void*)(&sbuff[0]),64,0,0);
  return sbuff[3];
}
...
{
  if (!sjpcm_inited) return;
  sif_call_rpc(&cd0,SJPCM_GETBUFFD,0,(void*)(&sbuff[0]),64,(void*)(&sbuff[0]),64,0,0);
  return sbuff[3];
}
...
void SjPCM_Quit()
{
	...
	sif_call_rpc(&cd0,SJPCM_QUIT,0,(void*)(&sbuff[0]),0,(void*)(&sbuff[0]),0,0,0);
	sjpcm_inited = 0;
}
